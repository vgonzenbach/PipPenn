...
library(dplyr)
library(stringr)
source("Code/Movelet_Quy.R") # Modified Movelet functions
```
...
convertTimeSeries = function(df){
  ...
  df$Wrist_Location = gsub(" ", "", df$Wrist_Location, fixed = TRUE) %>% factor()
  return(df)
}
...
```{r balance_LR, echo = FALSE}
## remove duplicate left and right wrist to create balance data
balance_LR = function(df){
  keep_idx <- c()
  diff_time <- diff(df$time)
  diff_wrist <- diff(ifelse(df$Wrist_Location == "left", 0, 1))
  ...
  nodup_df <- df[keep_idx, ]
  return(nodup_df)
}

nodup_dat = balance_LR(dat)

table(nodup_dat$Wrist_Location)
```

```{r}
## extract 3 pcs for the left hand and right hand
make_movelet_df = function(df, n.pcs = 3){
  
  # prepare activity name
  ActivityName = df$ActivityName[df$Wrist_Location=="left"]
  ActivityName = gsub(" ", "_", ActivityName)
  
  extract_PCs = function(){
    # Extract n PCs from each hand 
    pc_left = df %>% filter(Wrist_Location == "left") %>%  select(ends_with("coord")) %>% prcomp(center=TRUE, scale.=TRUE)
    pc_left = pc_left$x[,1:n.pcs]
    
    pc_right = df %>% filter(Wrist_Location == "right") %>% select(ends_with("coord")) %>% prcomp(center=TRUE, scale.=TRUE)
    pc_right = pc_right$x[,1:n.pcs]
    
    pcs_combined = data.frame(pc_left, pc_right)
    colnames(pcs_combined) = c("PC1_left","PC2_left","PC3_left","PC1_right","PC2_right","PC3_right")
    return(pcs_combined)
  }
  ## combine data with PCs
  
  movelet_df <- data.frame(extract_PCs(), ActivityName)
  return(movelet_df)
}

movelet.dat = make_movelet_df(nodup_dat)
```

```{r}
## 9._Med_Taking is about 24 seconds, about 1200 observations, so 1 sec = 50 obs. 

make_train_movelets = function(movelet.dat, subjectName="patient52"){
  
  # TODO: Select from and to times based on which patient
  
  patient <<- Acceleration_Create2(movelet.dat,
                                   subjectName=subjectName,
                                   n_axes=6,
                                   frequency=50)
            
  Ch.Pick_up_and_open_pill_bottle <<- Movelet_Create2(FROM = min(which(patient$Label == "9._Med_Taking")) + 200, # fix times
                                                      TO = min(which(patient$Label == "9._Med_Taking")) + 400, # fix times
                                                      DATA = patient, 
                                                      n_axes = 6,
                                                      ChapName = "Pick_up_and_open_pill_bottle")
  
  Ch.Pour_pill_return_bottle_and_take_pill <<- Movelet_Create2(FROM = min(which(patient$Label == "9._Med_Taking")) + 400, # fix times
                                                               TO = min(which(patient$Label == "9._Med_Taking")) + 500, # fix times
                                                               DATA = patient, 
                                                               n_axes = 6,
                                                               ChapName = "Pour_pill_return_bottle_and_take_pill")
  
  Ch.Wash_Hands <<- Movelet_Create2(FROM = min(which(patient$Label == "Wash_Hands")),
                                    TO = max(which(patient$Label == "Wash_Hands")),
                                    DATA = patient, n_axes = 6,
                                    ChapName = "Wash_Hands")
  
  Ch.Look_around_in_bag_or_purse <<- Movelet_Create2(FROM = min(which(patient$Label == "Look_around_in_bag_or_purse")),
                                                     TO = max(which(patient$Label == "Look_around_in_bag_or_purse")),
                                                     DATA = patient, n_axes = 6,
                                                     ChapName = "Look_around_in_bag_or_purse")
}

make_train_movelets(movelet.dat)

## train Look_around_in_bag_or_purse chapter

## test data
test.dat <- subset(movelet.dat, movelet.dat$ActivityName %in% c("Walk","Turn_Key","Open_Door","Eat_From_Bag",
                                                                "6._Med_Taking","7._Med_Taking","4._Med_Taking","5._Med_Taking",
                                                                "8._Med_Taking","10._Med_Taking","11._Med_Taking","12._Med_Taking"))

test.dat.patient.52 <- Acceleration_Create2(test.dat, 
                                            subjectName="patient52", 
                                            n_axes = 6, 
                                            frequency=50) 
...
saveRDS(pred, file = "Results/patient52_8_meds.rds")
...
```{r}
# load it
patient52_8_meds <- readRDS(file = "Results/patient52_8_meds.rds")
patient52_8_meds$Pred2 <- recode(patient52_8_meds$Pred, 
                                 `1`="Pick_up_and_open_pill_bottle", 
                                 `2`="Pour_pill_return_bottle_and_take_pill", 
                                 `3`="Wash_Hands", 
                                 `4`="Look_around_in_bag_or_purse")
```

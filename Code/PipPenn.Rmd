...
author: "Quy Cao and Haochang Shou"
...
act.name<-unique(dat$ActivityName)
...
print(unique(dat$ActivityName))
```

```{r plot, echo=FALSE}
wrist.type<-unique(dat$Wrist_Location)

wrist.r<-which(dat$Wrist_Location==wrist.type[1]) #right wrist
wrist.l<-which(dat$Wrist_Location==wrist.type[2])

dygraph(data=dat[wrist.r,c('time',names(dat)[c(8:10,30)])],main=paste('Patient', dat$PatientID[1],' Accel Right Wrist',sep=''))%>% 
  dyAxis("x", drawGrid = FALSE) %>%
  dyOptions(colors = RColorBrewer::brewer.pal(7, "Set1"))  %>%
  dyRangeSelector()

dygraph(data=dat[wrist.r,c('time',names(dat)[c(11:13,30)])],main=paste('Patient', dat$PatientID[1],' Rotation Right Wrist',sep=''))%>% 
  dyAxis("x", drawGrid = FALSE) %>%
  dyOptions(colors = RColorBrewer::brewer.pal(7, "Set1"))  %>%
  dyRangeSelector()

dygraph(data=dat[wrist.r,c('time',names(dat)[c(20:22,30)])],main=paste('Patient', dat$PatientID[1],' Attitude Right Wrist',sep=''))%>% 
  dyAxis("x", drawGrid = FALSE) %>%
  dyOptions(colors = RColorBrewer::brewer.pal(7, "Set1"))  %>%
  dyRangeSelector()

dygraph(data=dat[wrist.r,c('time',names(dat)[c(14:16,30)])],main=paste('Patient', dat$PatientID[1],' Gravity Right Wrist',sep=''))%>% 
  dyAxis("x", drawGrid = FALSE) %>%
  dyOptions(colors = RColorBrewer::brewer.pal(7, "Set1"))  %>%
  dyRangeSelector()

dygraph(data=dat[wrist.r,c('time',names(dat)[c(17:19,30)])],main=paste('Patient', dat$PatientID[1],' Magentic Right Wrist',sep=''))%>% 
  dyAxis("x", drawGrid = FALSE) %>%
  dyOptions(colors = RColorBrewer::brewer.pal(7, "Set1"))  %>%
  dyRangeSelector()
```
